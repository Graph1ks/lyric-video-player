...
### Legacy rehabilitation implementation state — PR #74–#80 merged; Laser moving-head rebuild active
...
- PR #79 is merged and permanently supersedes the rejected four-fountain Sparks identity. Its Laser Canopy rewrite was still rejected on real display because it read as thin projected rays rather than real animated disco lighting.
- Draft PR #81 replaces WORLD_02 with an explicit moving-head rig: eight visible ceiling fixtures, four independently aimed shafts per fixture, continuous fan/cross/orbit choreography, broad volumetric haze bodies, floor-hit blooms/scan rings and room wash. Audio remains light-only.
- PR #80 is merged and implements the two-way readability architecture: analytical world context → 220 ms metric smoothing → polarity hysteresis → shared `WorldTypographyTreatment` for ordinary and persistent lyrics.
